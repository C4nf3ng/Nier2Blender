...
class wmb3_vertex(object):
	...
	def __init__(self, wmb_fp , stride):
		...
		if stride > 0x14:									
			self.boneIndices = [to_int(wmb_fp.read(1)) for _ in range(4)]
		if stride > 0x18:									
			self.boneWeights = [to_int(wmb_fp.read(1))/255 for _ in range(4)]
...
class wmb3_vertexGroup(object):
	...
	def __init__(self, wmb_fp, faceSize):
		...
		for _ in range(self.vertexGroupHeader.vertexCount):
		...
		self.boneWeightArray.extend(
		    wmb3_boneWeight(wmb_fp, self.vertexGroupHeader.boneWeightStride)
		    for _ in range(self.vertexGroupHeader.vertexCount))
		self.faceRawArray = []
		wmb_fp.seek(self.vertexGroupHeader.faceArrayOffset)
		for _ in range(self.vertexGroupHeader.faceCount):
			if faceSize == 2:
...
class wmb3_boneSet(object):
	...
	def __init__(self, wmb_fp, boneSetCount):
		...
		for boneSetInfo in boneSetInfoArray:
			...
			boneSet = [to_int(wmb_fp.read(2)) for _ in range(boneSetInfo[1])]
...
class wmb3_meshGroup(object):
	...
	def __init__(self, wmb_fp):
		...
		self.materialIndexArray.extend(
		    to_int(wmb_fp.read(2)) for _ in range(materialIndexArrayCount))
		wmb_fp.seek(boneIndexArrayOffset)
		self.boneIndexArray.extend(
		    to_int(wmb_fp.read(2)) for _ in range(boneIndexArrayCount))
...
class WMB3(object):
	...
	def __init__(self, wmb_file):
		...
		wtp_fp = 0
		self.wta = 0
		wmb_fp = open(wmb_file, "rb") if os.path.exists(wmb_file) else 0
		...
		for _ in range(self.wmb3_header.meshCount):
		...
		self.boneMap = []
		self.boneMap.extend(
		    to_int(wmb_fp.read(4)) for _ in range(self.wmb3_header.boneMapCount))
	...
	def clear_unused_vertex(self, meshArrayIndex,vertexGroupIndex):
		...
		mappingDict = {
		    usedVertexIndexArray[newIndex]: newIndex
		...
		for newIndex in range(len(usedVertexIndexArray)):
			...
			if self.hasBone:
				...
				if bonesetIndex < 0xffffffff:
					...
					s = sum(list(meshVertices[i].boneWeights))
