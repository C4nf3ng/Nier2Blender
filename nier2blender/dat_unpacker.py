...
def extract_file(fp, filename, FileOffset, Size, extract_dir):
    ...
    with open(f'{extract_dir}/{filename}', 'wb') as outfile:
        print(f"extracting file {filename} to {extract_dir}/{filename}")
        outfile.write(FileContent)
    if filename.find('wtp') > -1:
        with open(f'{extract_dir}/{filename}', "rb") as wtp_fp:
            content = wtp_fp.read(Size)
            dds_group = content.split(b'DDS ')
            dds_group = dds_group[1:]
            for i in range(len(dds_group)):
                print(
                    f"unpacking {filename} to {extract_dir}/{filename.replace('.wtp', '_%d.dds'%i)}"
                )

                with open(f'{extract_dir}/' + filename.replace('.wtp','_%d.dds'%i), "wb") as dds_fp:
                    dds_fp.write(b'DDS ')
